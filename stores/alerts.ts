...
export const useAlertsStore = defineStore('alerts', () => {
    ...
    function addAlert({ title, message, type, button, displayTime }: { title: string, message: string, type: 'success' | 'error' | 'info', button?: { text: string; action: () => void }, displayTime?: number }): number {
    ...
    function dismiss(id: number) {
        alerts.value.splice(alerts.value.findIndex(alert => alert.id === id), 1);
    }
