...
export const useServicesAuthStore = defineStore("servicesAuth", () => {
    ...
    const calendarRdy = computed(
        () => gapiLoaded.value && gisLoaded.value && googleAuth.value
    );
    ...
    function getZebraApiKey() {
        ...
        if (zebraApiKey.value === "") {
            ...
            useAlertsStore().addAlert({
                title: "Zebra API key is empty",
                message: "Please set the Zebra API key in the settings",
                type: "error",
            });
        }
